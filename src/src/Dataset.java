...
public class Dataset{
	...
	// Customer evaluation: 0 is bad (< 3), 1 is good (>= 3)
	public static List<String> convertValues(List<String> values){
		List<String> toRet = new ArrayList<String>();
	...
	// Create a arff file
	public static void saveFile(List<List<String>> values, HashMap<String, Integer> words, List<String> valuations){
		try{
			...
			writer.println("@attribute valuation {0, 1}");
			
			writer.println("@data");
			
			for(int i = 0; i < values.size() - 1; i++){
				for(int j = 0; j < values.get(i).size(); j++){
					writer.print(values.get(i).get(j) + ",");
				}
				writer.println(valuations.get(i));
			}
